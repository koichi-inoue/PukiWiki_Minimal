...
function plugin_youtube_convert() {
  ...
  $str_aspect = strval( $aspect * 100.0 )."%";
	...
	for($i=0; $i<5; $i++){
		if( $args[$i] == "auto") $autoplay = 'autoplay=1&mute=1';
		if( $args[$i] == "loop") $loop = 'loop=1&playlist='.$name;
	}
  ...
  return <<<HTML
    ...
    <div id="$id">
      <iframe src="https://www.youtube.com/embed/$name?$autoplay&autohide=1&$loop&theme=dark&color=white&rel=0" $iframeStyle frameborder="0" allowfullscreen></iframe>
    </div>
HTML;
